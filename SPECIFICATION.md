...
### 3.3 音声エンコーダ別 品質選択肢
- **qaac**:
  - `AAC-LC 高品質 (TVBR 110 / 約192kbps)`
  - `AAC-LC 標準   (TVBR 90  / 約128kbps - 推奨)`
  - `HE-AAC 低容量 (CVBR 48k / 音質重視配信)`
  - `カスタム TVBR...` (1〜127) / `カスタム CVBR...` (kbps)
- **neroAacEnc**:
  - `高品質 (-q 0.50)` / `標準 (-q 0.40)` / `カスタム -q...` (0.20〜0.65)
- **fdkaac**:
  - `最高品質 (VBR 5)` / `標準品質 (VBR 4)` / `カスタム VBR...` (1〜5)
- **FFmpeg内蔵 (Opus / AAC / Vorbis)**:
  - `64 kbps`, `96 kbps`, `128 kbps (推奨)`, `160 kbps`, `192 kbps`, `256 kbps`, `320 kbps`, `カスタム...`
...
### 6.8 ハードウェア検出 & デコード/転送制御 (`internal/core/hw_scanner.go`, `hw_compat.go`)
...
### 6.9 プラットフォーム容量逆算ロジック (`internal/core/bitrate_calc.go`)
- **計算式**:
...
### 6.10 インターレース検出 & 解除 (`internal/core/deinterlace.go`)
...
### 6.11 外部音声エンコーダ & 特殊チャンネル処理 (`internal/core/audio_pipeline.go`, `audio_mapping.go`)
- **外部エンコーダパイプライン**:
...
### 6.12 チャプター & 字幕分割エンジン (`internal/core/split_engine.go`)
- **分割ソース**:
...
### 6.13 メタデータ & 編集 & DASH PTS補正 (`internal/core/metadata.go`)
...
### 7.1 CPU 制限 (P-Core / E-Core / EcoQoS) (`internal/runner/cpu_control.go`)
- **Windows API 連携**:
...
### 7.2 Windows タスクバー進捗表示 (`internal/runner/taskbar.go`)
...
### 7.3 プロセス一時停止 / 再開 (`internal/runner/process.go`)
- `NtSuspendProcess` / `NtResumeProcess` により、FFmpeg プロセスを即座に一時停止 / 再開。

### 7.4 エクスプローラー連携ワンクリック登録 (`internal/ui/context_menu.go`)
