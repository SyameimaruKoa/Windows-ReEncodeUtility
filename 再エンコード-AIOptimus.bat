@echo off
rem ffmpeg一括再エンコードVer9.1 (わっち改修版・中間ファイルモード追加)
...
set "losslesscut_path=C:\Users\kouki\OneDrive\PortableApps\LosslessCutPortable\LosslessCutPortable.exe"
...
echo   2. テンプレートから選択
echo   3. 中間ファイル作成モード (高画質・MKV・音声コピー)
echo.
choice /c 123 /m "モードを選択してください"
if %errorlevel%==1 goto InteractiveSetup
if %errorlevel%==2 goto TemplateSelect
if %errorlevel%==3 goto IntermediateMode
goto root
...
rem --- [新規追加] 中間ファイル作成モード ---
:IntermediateMode
cls
echo --- 中間ファイル作成モード ---
echo 中間ファイル用のエンコードオプションを設定します...
call "%~dp0ffmpegエンコードオプション-AIOptimus.bat" "intermediate"
if "!encoder!"=="" (
    echo エラー: エンコードオプションが設定されませんでした。処理を中止します。
    pause
    exit /b
...
echo.
echo --- 中間ファイルモードの固定設定 ---
echo 以下の設定が自動的に適用されました。
set "extension=mkv"
echo   - 出力ファイルの拡張子: .mkv
set "AudioEncode=copy"
echo   - 音声: そのままコピー (-c:a copy)
set "output_mode=subfolder"
echo   - 出力先: 各動画ファイルの存在するフォルダ内の「encoded_output」サブフォルダ
set "cut=no"
echo   - 動画カット: しない
set "exiftool=yes"
echo   - メタデータ: ExifToolで全コピー
set "after_process_action=none"
echo   - 完了後アクション: 何もしない
set "vf="
set "argument="
